function initTable() {
    ...
    $a.bootstrapTable({
        ...
        url: "/try/show_instore", //获取数据的Servlet地址
        toolbar: '#toolbar',
        striped: true,  //表格显示条纹
        ...
        columns:[
            ...
            },
            {
                formatter: function (value, row, index) {
                    debugger;
                    return [
                        '<a href="javascript:delPer(' + row.Iid + ')">' +
...
function delPer(Iid) {
    ...
    $.ajax(
        {
            type: "POST",
            url: "/try/delete_instore"+"?time="+new Date().getTime(),
            async:true,
            contentType: "application/json; charset=utf-8",
            data: s,
            dataType: "json",
...
function initlayer() {
    ...
    $("#btn_add").click(function () {
        ...
        $(".register_form").easyform({
            ...
            success : function  () {
                debugger;
                $setIid=$("input[name='Iid']");
                $setInum=$("input[name='Inum']");
                $setDate=$("input[name='Idate']");
                $setPrice=$("input[name='Iprice']");
                var Iid=$setIid.val();
                var Gid=$("select[name='Gid']").val();
                var Idate=$setDate.val();
                var Sid=$("select[name='Sid']").val();
                var Inum=$setInum.val();
                var Iprice=$setPrice.val();

                var data={"Iid":Iid,"Gid":Gid,"Sid":Sid,"Idate":Idate,"Inum":Inum,"Iprice":Iprice};
                ...
                if(countAddAjaxI===0){
                    ...
                    $.ajax(
                        {
                            type: "POST",
                            url: "/try/add_instore"+"?time="+new Date().getTime(),
                            async:true,
                            contentType: "application/json; charset=utf-8",
                            data: s,
                            dataType: "json",
